...
const pageQueue = new Queue('pageQueueKrishaDaily', { connection: redisConnection });
const apartmentQueue = new Queue('apartmentQueueKrishaDaily', { connection: redisConnection });
...
async function scrapePage(job: Job<{ pageUrl: string }>): Promise<void> {
    ...
    try {
        ...
        console.log(`Queued ${links.length} apartments from ${pageUrl} on krisha daily almaty page`);
    } catch (error) {
...
async function krishaParseDailyAlmaty(): Promise<void> {
    try {
        ...
        while (!isLastPage && currentPage <= Number(process.env.PARSER_PAGE_LIMIT)) {
            const pageUrl = `https://krisha.kz/arenda/kvartiry-posutochno/almaty/?das[_sys.hasphoto]=1&das[who]=1&rent-period-switch=%2Farenda%2Fkvartiry-posutochno&page=${currentPage}`;
        ...
        console.error('Error in krishaParseDailyAlmaty:', error);
    } finally {
...
const apartmentWorker = new Worker('apartmentQueueKrishaDaily', async job => {
