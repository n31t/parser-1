...
const pageQueue = new Queue('pageQueueNedvizhkaBuy', { connection: redisConnection });
const apartmentQueue = new Queue('apartmentQueueNedvizhkaBuy', { connection: redisConnection });
...
async function scrapePage(job: Job<{ pageUrl: string }>): Promise<void> {
    ...
    try {
        ...
        console.log(`Queued ${links.length} apartments from ${pageUrl} on nedvizhka buy almaty`);
    } catch (error) {
...
async function nedvizhkaParseBuyAlmaty(): Promise<void> {
    try {
        await createBrowser(); 
        let currentPage = 1;
        ...
        while (!isLastPage && currentPage <= Number(process.env.PARSER_PAGE_LIMIT)) {
            const pageUrl = `https://nedvizhka.kz/posts/kvartiry-prodazha/almaty?page=${currentPage}&viewType=list&onlyComplexLayouts=0&personal=1#`;
        ...
        console.error('Error in nedvizhkaParseBuyAlmaty:', error);
    } finally {
        const currentDate = new Date();
        const indexName = "homespark3";
        const index = pinecone.index(indexName);
        await deleteOlderThanDate(index, currentDate, "buy", "nedvizhka");
        await browser!.close();
    }
...
function startApartmentWorker() {
    if (!apartmentWorker) {
        apartmentWorker = new Worker('apartmentQueueNedvizhkaBuy', async job => {
...
const pageWorker = new Worker('pageQueueNedvizhkaBuy', async job => {
...
export default nedvizhkaParseBuyAlmaty;