...
const pageQueue = new Queue('pageQueueKnBuy', { connection: redisConnection });
const apartmentQueue = new Queue('apartmentQueueKnBuy', { connection: redisConnection });
...
async function scrapePage(job: Job<{ pageUrl: string }>): Promise<void> {
    ...
    try {
        ...
        console.log(`Queued ${links.length} apartments from ${pageUrl} on kn buy almaty`);
    } catch (error) {
...
async function knParseBuyAlmaty(): Promise<void> {
    try {
        ...
        while (!isLastPage && currentPage <= 10) {
            const pageUrl = `https://www.kn.kz/almaty/prodazha-kvartir-bez-posrednikov/page/${currentPage}/`;
        ...
        console.error('Error in knParseBuyAlmaty:', error);
    } finally {
...
function startApartmentWorker() {
    if (!apartmentWorker) {
        apartmentWorker = new Worker('apartmentQueueKnBuy', async job => {
...
const pageWorker = new Worker('pageQueueKnBuy', async job => {
