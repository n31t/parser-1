...
const pageQueue = new Queue('pageQueueKrishaRent', { connection: redisConnection });
const apartmentQueue = new Queue('apartmentQueueKrishaRent', { connection: redisConnection });
...
async function scrapePage(job: Job<{ pageUrl: string }>): Promise<void> {
    ...
    try {
        ...
        console.log(`Queued ${links.length} apartments from ${pageUrl} on krisha rent almaty page`);
    } catch (error) {
...
async function krishaParseRentAlmaty(): Promise<void> {
    try {
        let currentPage = 1;
        let isLastPage = false;

        while (!isLastPage && currentPage <= Number(process.env.PARSER_PAGE_LIMIT)) {
            const pageUrl = `https://krisha.kz/arenda/kvartiry/almaty/?das[_sys.hasphoto]=1&das[live.furniture][0]=1&das[live.furniture][1]=2&das[who]=1&rent-period-switch=%2Farenda%2Fkvartiry&page=${currentPage}`;
        ...
        console.error('Error in krishaParseRentAlmaty:', error);
    } finally {
...
// Set up workers
const pageWorker = new Worker('pageQueueKrishaRent', async job => {
    await scrapePage(job);
}, { connection: redisConnection, concurrency: 1 });

const apartmentWorker = new Worker('apartmentQueueKrishaRent', async job => {
...
export default krishaParseRentAlmaty;