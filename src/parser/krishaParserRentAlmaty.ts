...
async function cleanUpOldPineconeEntries(index, currentDate) {
    ...
    const embeddedPrompt = await new GoogleGenerativeAIEmbeddings().embedQuery('delete old vectors from Pinecone.');
        ...
        for (const id of allIds) {
            ...
            if (metadata && metadata.site === "krisha" && metadata.type === "rent" && new Date(metadata.lastChecked).getTime() < deleteOlderThanDate.getTime()) {
                idsToDelete.push(id);
            }
        }
...
async function scrapeCurrentPage(page: Page, data: Data[]): Promise<void> {
    ...
    for (const link of links) {
        ...
        try {
            ...
            await new Promise(resolve => setTimeout(resolve, getRandomDelay(4000, 8000))); // Random delay between 40 to 80 seconds
        }finally {
...
async function scrapeAllPages(page: Page, data: Data[], currentPage: number = 1): Promise<void> {
    ...
    while (!isLastPage) {
        ...
        try {
            await page.goto(`https://krisha.kz/arenda/kvartiry/almaty/?das[_sys.hasphoto]=1&das[live.furniture][0]=1&das[live.furniture][1]=2&das[who]=1&rent-period-switch=%2Farenda%2Fkvartiry&page=${currentPage}`);
...
async function krishaParseRentAlmaty(): Promise<Data[]> {
