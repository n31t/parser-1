...
const pageQueue = new Queue('pageQueueKnRent', { connection: redisConnection });
const apartmentQueue = new Queue('apartmentQueueKnRent', { connection: redisConnection });
...
async function scrapePage(job: Job<{ pageUrl: string }>): Promise<void> {
    ...
    try {
        ...
        console.log(`Queued ${links.length} apartments from ${pageUrl} on kn rent almaty`);
    } catch (error) {
...
async function knParseRentAlmaty(): Promise<void> {
    try {
        ...
        while (!isLastPage && currentPage <= 10) {
            const pageUrl = `https://www.kn.kz/almaty/arenda-kvartir/page/${currentPage}/`;
...
function startApartmentWorker() {
    if (!apartmentWorker) {
        apartmentWorker = new Worker('apartmentQueueKnRent', async job => {
...
const pageWorker = new Worker('pageQueueKnRent', async job => {
